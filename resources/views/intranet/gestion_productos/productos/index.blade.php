        ...
        <div class="col-12 col-md-8 p-0">
            ...
            <div class="p-4" id="layoutLeft">
                <div class="form-group">
                    <input type="text" v-model="sBuscar" class="form-control" placeholder="Buscar por nombre, descripción, categoría o línea">
                </div>
                <table class="table table-bordered table-hover" id="tblProductos">
                    <thead>
                        <tr>
                            <th class="bg-primary">Nombre</th>
                            <th class="bg-primary">Categor&iacute;as</th>
                            <th class="bg-primary">L&iacute;neas</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white">
                        <tr v-for="producto of lstProductosFiltrados" v-bind:class="{selected: iIdSeleccionado === producto.id}" v-on:click="panelEditar(producto.id)" style="cursor: pointer;" v-cloak>
                            ...
                            <td>
                                <label class="m-0" v-for="(categoria, i) in producto.categorias" v-cloak>@{{ categoria.nombre_es }}<span v-if="i < producto.categorias.length - 1">,&nbsp;</span></label>
                            </td>
                            <td>
                                <label class="m-0" v-for="(linea, i) in producto.lineas" v-cloak>@{{ linea.nombre_espanol }}<span v-if="i < producto.lineas.length - 1">,&nbsp;</span></label>
                            </td>
                        </tr>
...
@section('js')
    <script src="/js/intranet/productos.js?cvcn=14"></script>
@endsection
