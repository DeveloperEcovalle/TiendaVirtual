                ...
                <table class="table table-bordered table-hover" id="tblUsuarios">
                    <thead>
                        <tr>
                            ...
                            <th class="bg-primary">Nombres y Apellidos</th>
                            <th class="bg-primary">Username</th>
                            <th class="bg-primary text-center">Registro</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white">
                        <tr v-for="usuario of lstUsuarios" v-bind:class="{selected: iIdSeleccionado === usuario.id}" v-on:click="panelEditar(usuario.id)" style="cursor: pointer;" v-cloak>
                            <td class="text-center">@{{ usuario.id_interno }}</td>
                            <td>@{{ usuario.persona.nombres + ' ' + (usuario.persona.apellido_1 ? usuario.persona.apellido_1 : '') + ' ' + (usuario.persona.apellido_2 ? usuario.persona.apellido_2 : '') }}</td>
                            <td>@{{ usuario.username }}</td>
                            <td class="text-center">@{{ usuario.fecha_reg }}</td>
                        </tr>
                        <tr v-if="lstUsuarios.length === 0" v-cloak>
...
@section('js')
    <script src="/js/intranet/usuarios.js?cvcn=14"></script>
@endsection
